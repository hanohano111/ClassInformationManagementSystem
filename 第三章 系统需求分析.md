...
### 3.2 用户功能 
当用户以游客身份打开微信小程序时，系统会引导其完成登录，并获取 OpenID 用于身份识别。登录完成后，用户在后续使用小程序时无需重复登录。用户可浏览与自身相关的班级事务功能：在个人账户中查询与修改个人信息；在“消息”界面查看各班级的通知、作业、请假、签到等消息汇总；在“班级”界面查看已加入的班级及相关信息，从而实现对班级信息的管理与查看。
...
flowchart LR
 ...
 UC_MESSAGE -.《include》.-> UC_VIEW_NOTICE
 UC_MESSAGE -.《include》.-> UC_VIEW_ASSIGN
 UC_MESSAGE -.《include》.-> UC_VIEW_LEAVE
 UC_MESSAGE -.《include》.-> UC_CHECKIN
 UC_CLASS -.《include》.-> UC_JOIN
 UC_CLASS -.《include》.-> UC_EXIT
 UC_CLASS -.《include》.-> UC_HIDE
 UC_NOTICE -.《include》.-> UC_VIEW_NOTICE
 UC_ASSIGN -.《include》.-> UC_VIEW_ASSIGN
 UC_ASSIGN -.《include》.-> UC_SUBMIT_ASSIGN
 UC_LEAVE -.《include》.-> UC_APPLY_LEAVE
 UC_LEAVE -.《include》.-> UC_VIEW_LEAVE
 UC_ATTEND -.《include》.-> UC_CHECKIN
```
...
- 扩展路径：
  - 1a. 用户未勾选协议：页面提示“请先同意《协议条款》”，并阻止登录操作
  - 4a. 云函数调用失败或返回错误（如无法获取 OPENID、用户不存在等）：页面提示错误信息（如“登录失败，请重试”），停留在登录页面，允许用户重试
...
- 用例描述：加入班级
- 执行者：用户
- 前置条件：用户已登录且已完成必要身份信息绑定（若系统要求）
- 后置条件：班级列表新增该班级，用户可访问该班级相关功能
- 基本路径：
  1. 用户进入“班级”页面，点击加入班级
  2. 输入班级码
...
sequenceDiagram
    ...
    alt 确认隐藏
        ...
        云函数-->>列表页: 返回班级列表数据
        列表页-->>用户: 按 hiddenClassIds 隐藏班级卡片
    else 取消
        信息页-->>用户: 不隐藏，保持当前页面
    end
```
...
- 执行者：用户
- 前置条件：用户已登录，且已加入目标班级；用户进入班级信息页面
- 后置条件：删除该用户与班级的成员关系；返回班级列表后不再显示该班级
- 基本路径：
  1. 用户进入班级信息页面，点击“退出班级”
  2. 系统弹出确认对话框
  3. 用户确认退出，前端调用云函数 `exitClass(classId)`
  4. 云函数从微信上下文获取 `OPENID`，在成员表中查找该用户与班级的成员记录
  5. 删除成员记录并返回退出结果
  6. 前端提示退出成功并返回班级列表，列表页重新加载后不再显示该班级
- 扩展路径：
  - 3a. 用户取消操作：不调用云函数，页面保持不变
...
- 用例描述：提交作业
- 执行者：用户
- 前置条件：用户已加入班级且作业在截止时间内
- 后置条件：系统记录用户提交内容/附件并更新提交状态
- 基本路径：
  1. 用户进入作业列表并打开指定作业详情
  2. 填写作业内容/上传附件
  3. 点击提交
  4. 后台保存作业提交记录（**注意：当前实现中，作业提交内容 `content` 未加密，直接存储**）
  5. 页面显示“已提交”状态与提交时间
...
> **加密说明**：当前实现中，作业提交内容（`content`）未加密，直接存储到数据库。前端提交时未进行加密处理，云函数直接保存明文内容。
...
- 基本路径：
  ...
  2. 填写请假日期、原因、上传证明（可选）
  3. 点击提交
  4. 后台保存请假申请（状态为待审批），**注意：当前实现中，请假原因 `reason` 未加密，直接存储**
  5. 教师端可在请假列表/消息中心查看待审批记录
  6. 返回请假列表页面，页面重新加载并显示请假记录与审批状态
...
sequenceDiagram
    ...
    alt 提交成功
        ...
        云函数-->>列表页: 返回请假列表(含状态)
        列表页-->>用户: 展示请假记录与审批状态
        note over 消息页: 管理员/用户进入“消息”页时\ngetMessages 会根据 leave_requests 生成待审批/状态变化消息
    else 提交失败
        创建页-->>用户: 提示“提交失败/原因不能为空/日期不能为空”等
    end
```

> **加密说明**：当前实现中，请假原因（`reason`）未加密，直接存储到数据库。前端提交时未进行加密处理，云函数直接保存明文内容。

### 3.2.6 签到功能

- 用例描述：签到码签到（4位数字）
- 执行者：用户
- 前置条件：管理员已生成有效签到码（默认有效期 5 分钟）；用户已登录并进入签到页面
...
sequenceDiagram
    ...
    note over 用户,页面: 前置：管理员已生成有效签到码（5分钟内）
    用户->>页面: 打开签到页面，输入4位签到码
    用户->>页面: 点击“确认签到”
...
- 用例描述：查看各班级消息汇总
- 执行者：用户
- 前置条件：用户已登录并已加入至少一个班级
- 后置条件：用户可在消息中心查看所有班级的通知、作业、请假、签到等消息汇总，并可点击跳转到对应详情页
- 基本路径：
  1. 用户进入“消息”页面（TabBar 第一个入口）
  2. 前端调用云函数 `getMessages`，云函数汇总该用户所有班级的近一周消息（通知、作业、请假、签到等）
  3. 页面按时间倒序展示消息列表，每条消息显示班级名称、消息类型、标题、内容摘要、时间
  4. 用户点击某条消息，根据消息类型跳转到对应的详情页（通知详情/作业详情/请假详情/签到页面）
  5. 用户可对消息进行归档或删除操作（仅本地生效）
...
flowchart LR
 ...
 subgraph SYS[班级信息管理系统]
   ...
   UC_CREATE_CLASS((创建班级))
   UC_ADD_MEMBER((添加成员))
   UC_REMOVE_MEMBER((移除成员))
   UC_SET_ADMIN((设置管理员))
   UC_CREATE_NOTICE((发布通知))
   UC_DELETE_NOTICE((删除通知))
   UC_CREATE_ASSIGN((发布作业))
   UC_DELETE_ASSIGN((删除作业))
   UC_GRADE_ASSIGN((批改作业))
   UC_APPROVE_LEAVE((审批请假))
   UC_GEN_CHECKIN((生成签到码))
   UC_VIEW_RECORDS((查看签到记录))
 end
 ...
 UC_MESSAGE -.《include》.-> UC_CREATE_NOTICE
 UC_MESSAGE -.《include》.-> UC_DELETE_NOTICE
 UC_MESSAGE -.《include》.-> UC_CREATE_ASSIGN
 UC_MESSAGE -.《include》.-> UC_DELETE_ASSIGN
 UC_MESSAGE -.《include》.-> UC_GRADE_ASSIGN
 UC_MESSAGE -.《include》.-> UC_APPROVE_LEAVE
 UC_MESSAGE -.《include》.-> UC_GEN_CHECKIN
 UC_MESSAGE -.《include》.-> UC_VIEW_RECORDS
 UC_CLASS -.《include》.-> UC_CREATE_CLASS
 UC_MEMBER -.《include》.-> UC_ADD_MEMBER
 UC_MEMBER -.《include》.-> UC_REMOVE_MEMBER
 UC_MEMBER -.《include》.-> UC_SET_ADMIN
 UC_NOTICE -.《include》.-> UC_CREATE_NOTICE
 UC_NOTICE -.《include》.-> UC_DELETE_NOTICE
 UC_ASSIGN -.《include》.-> UC_CREATE_ASSIGN
 UC_ASSIGN -.《include》.-> UC_DELETE_ASSIGN
 UC_ASSIGN -.《include》.-> UC_GRADE_ASSIGN
 UC_LEAVE -.《include》.-> UC_APPROVE_LEAVE
 UC_ATTEND -.《include》.-> UC_GEN_CHECKIN
 UC_ATTEND -.《include》.-> UC_VIEW_RECORDS
```
...
- 执行者：管理员（班级创建者）
- 前置条件：管理员已登录
- 后置条件：系统生成班级信息与班级码，创建者自动成为该班级管理员，班级出现在班级列表中
- 基本路径：
  1. 管理员进入班级列表页面，点击“创建班级”入口
  2. 填写班级名称、老师名称、学期等信息
...
- 后置条件：请假状态更新为同意/拒绝，用户端可查看审批结果
- 基本路径：
  1. 进入请假列表，选择待审批申请
  2. 点击同意/拒绝并填写审批意见
  3. 点击提交
  4. 前端调用云函数 `approveLeaveRequest`，后台校验管理员权限并更新请假记录状态（**注意：当前实现中，审批意见 `comment` 未加密，直接存储**）
  5. 页面显示更新后的请假详情与状态；用户端可在请假列表/消息中心看到状态变化
...
- **身份认证**：用户必须通过微信 OpenID 登录后才能访问系统功能。
- **角色权限**：系统采用基于角色的访问控制（RBAC），区分普通用户（member）与管理员（admin）权限。
- **管理员校验**：所有管理员操作（发布通知、发布作业、审批请假、生成签到码等）需校验当前用户在目标班级中的 `role='admin'` 或班级创建者身份。
- **数据隔离**：不同班级的数据相互隔离，成员仅可访问所属班级数据；用户只能查看和操作与自身相关的记录（如本人的请假申请、作业提交）。

#### 3.4.2.2 敏感数据保护

依据《中华人民共和国个人信息保护法》及相关规范，系统对个人敏感信息进行分级保护：

| 敏感等级 | 数据字段 | 加密要求 | 当前实现状态 |
|---------|---------|---------|-------------|
| **高敏感** | 手机号（phone）、学号/工号（studentNo） | 必须加密存储，查询时按需解密 | ✅ 已实现 AES-256 加密 |
| **高敏感** | 姓名（name）、教师名称（teacherName） | 必须加密存储 | ✅ 已实现 AES-256 加密 |
| **中敏感** | 学院（college）、专业（major） | 建议加密存储 | ✅ 已实现 AES-256 加密 |
| **中敏感** | 请假原因（reason）、审批意见（comment） | 建议加密存储 | ⚠️ 当前未加密，待后续优化 |
| **中敏感** | 作业提交内容（content）、成绩评语（grade/comment） | 建议加密存储 | ⚠️ 当前未加密，待后续优化 |
| **低敏感** | 班级名称、通知标题/内容 | 可明文存储 | - |

**加密算法说明**：
- 加密算法：AES-256-CBC
- 密钥管理：加密密钥仅存储于云函数服务端，前端不保存密钥
- IV 处理：每个加密字段使用独立随机 IV，IV 与密文一同存储（字段名加 `_iv` 后缀）
- 哈希索引：手机号额外生成 SHA-256 哈希值（`phone_hash`），用于快速查询

#### 3.4.2.3 敏感数据处理流程

下图展示敏感数据在系统中的加密/解密流程：

```mermaid
flowchart TB
    subgraph 前端["前端（小程序）"]
        A1[用户输入敏感信息<br/>姓名/学号/手机号等]
        A2[调用 encryptFields 加密]
        A3[发送加密数据到云函数]
    end

    subgraph 云函数["云函数（服务端）"]
        B1[接收加密数据]
        B2{需要读取明文？}
        B3[调用 decryptFieldsFromDB 解密]
        B4[业务处理]
        B5[调用 encryptFieldsForDB 加密]
        B6[写入/更新数据库]
        B7[返回结果]
    end

    subgraph 数据库["云数据库"]
        C1[(加密存储<br/>字段: xxx<br/>IV: xxx_iv)]
    end
...
- 所有数据传输通过微信小程序官方通道，底层采用 HTTPS 加密。
- 云函数调用基于微信云开发安全机制，自动校验调用来源。

#### 3.4.2.5 位置隐私

- 定位签到功能需获得用户明确授权（`wx.authorize`）。
- 位置数据仅用于签到校验，不长期存储用户精确位置。
- 当前版本签到页面未开放定位签到入口，仅支持签到码签到。

### 3.4.3 业务约束条件

#### 3.4.3.1 班级码生成规则

- **格式**：6 位字符，由数字（0-9）和大写英文字母（A-Z）随机组成
- **生成时机**：创建班级时由前端 `generateClassCode()` 函数生成
- **唯一性**：云函数写入前校验班级码在 `courses` 表中是否唯一，若重复则返回错误提示用户重新生成
- **有效期**：永久有效，班级存在期间不变

#### 3.4.3.2 签到码生成规则

- **格式**：4 位纯数字（0000-9999）
- **生成时机**：管理员点击"生成签到码"时由前端随机生成
- **有效期**：默认 5 分钟（300 秒），过期后签到码失效
- **唯一性**：同一班级同一时间只有一个有效签到码，生成新码时自动使旧码失效
- **校验规则**：签到时校验 `courseId + code` 且 `expireTime > 当前时间`

#### 3.4.3.3 其他业务规则

| 业务场景 | 约束条件 |
|---------|---------|
| 加入班级 | 用户不能重复加入同一班级；需提供正确的 6 位班级码 |
| 退出班级 | 班级创建者不能退出自己创建的班级 |
| 移除成员 | 不能移除班级创建者；不能移除自己 |
| 作业提交 | 截止时间前提交为正常提交，截止时间后提交标记为"迟交" |
| 请假审批 | 只有管理员可以审批；审批后状态变更为"同意"或"拒绝"，不可撤回 |
| 签到 | 同一签到码每个用户只能签到一次，重复签到返回"已签到" |
...
本章从用户角色、用户与管理员的功能需求、以及性能与安全等非功能需求角度，系统化给出了本项目的需求分析，并以“信息删除”等关键管理用例展示了用例描述、基本路径与简要时序图，为后续系统设计、实现与测试提供依据。

  ...
  try {
    ...
    if (!isAdmin) {
      try {
        ...
        console.error('[removeClassMember] 查询班级信息失败:', e);
      }
    } else {
      try {
        const courseRes = await courses.doc(classId).get();
    ...
    if (!isAdmin) {
      return { code: 403, success: false, message: '无权限删除成员' };
    }
    ...
    if (creatorOpenid && target.openid === creatorOpenid) {
      return { code: 400, success: false, message: '不能踢出班级创建者' };
    }

    // 保护：不能踢出自己（自己退出请用退出班级）
    if (target.openid === openid) {
      return { code: 400, success: false, message: '不能踢出自己，请使用退出班级' };
    }

    await courseMembers.doc(memberId).remove();

    return { code: 200, success: true, message: '已踢出该成员' };
  } catch (e) {
    console.error('removeClassMember error:', e);
    return { code: 500, success: false, message: '删除成员失败', error: e.message };
  }
};

...
exports.main = async (event) => {
  try {
    ...
    const { classId, title, content, attachments } = event;

    if (!classId) {
      return { code: 400, success: false, message: '课程ID不能为空' };
    }
    if (!title || !title.trim()) {
      return { code: 400, success: false, message: '通知标题不能为空' };
    }
    ...
    if (!isAdmin) {
      try {
        ...
        console.error('createNotice 查询课程信息失败:', e);
      }
    }

    if (!isAdmin) {
      return { code: 403, success: false, message: '无权限创建通知' };
    }
    ...
    const addRes = await notices.add({ data: noticeData });
    ...
    return { code: 500, success: false, message: '创建通知失败', error: e.message };
  }
};

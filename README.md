...
> 📖 详细的API接口规范请参考：[API接口设计文档](docs/api-design.md)
...
> 加密细节与使用方式可参考 `utils/crypto.js`、各云函数目录下的 `common/aes.js`。详细的加密实现指南请参考：[实现指南文档](docs/implementation-guide.md)
...
在云开发控制台创建前述集合，并配置合适的索引（至少包含 `users.openid` 唯一索引、`course_members(courseId, openid)` 复合索引等）。
...
> 📖 完整的系统需求分析请参考：[系统需求分析文档](docs/system-requirement-analysis.md)
